...
Record source approval decisions here as real university sources are reviewed.
...
| wikimedia-commons | approved | Reviewed university image assets, file-level license metadata, source URLs, and attribution. | Approved for image asset batches only when attribution is retained in `data/assets.json`. |

No approved source has been registered yet for canonical faculty, program, or ranking rows.
