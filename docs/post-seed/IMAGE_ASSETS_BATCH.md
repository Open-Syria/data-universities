...
3. Copy approved items into an approved local manifest with `approved: true`.
4. Generate variants with `build-image-assets`.
5. Run `upload-image-assets --dry-run`.
6. Inspect object keys and public URLs.
7. Upload to R2 only after review.
8. Add CDN URLs to canonical data only after deciding the public image schema.
